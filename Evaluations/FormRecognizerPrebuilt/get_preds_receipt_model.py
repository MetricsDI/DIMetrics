...
if __name__ == '__main__':
    ...
    for i in os.listdir(INPUT_DIR):
        if i != '.DS_Store':
            ...
            output_file = os.path.join(OUTPUT_DIR, "CORD_001_test_"+i[:-4]+'.json')
            ## Change file_type based on files in the folder
            # print(input_file)
            runAnalysis(input_file, output_file, file_type='image/png')
            print("Predictions generated for {} at {} in {:.3f} seconds".format(input_file, output_file, time.time() - cur))
            cur = time.time()
            time.sleep(6)
    print('*'*100)
    print("Total time Elapsed {:.3f}".format(time.time() - total_start))
